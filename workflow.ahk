...
if A_ScriptFullPath = A_LineFile {
    ...
    ocr_clip(*) {
        ...
        Run('cmd /c edge-playback -t"' temptxt '" -v zh-CN-XiaochenMultilingualNeural --rate="+20%"') ;使用rate增加减少播放速率
    ...
    ocr_file(*) {
        loop files "input\*.txt", "R" {
            temp := FileOpen(A_LoopFileFullPath,"r","utf-8") 
            ; 建立一个新的临时文件来读取文本。文件名固定但是每次需要重写文件
            RunWait('cmd /c edge-playback -f "' temp '" -v zh-CN-XiaochenMultilingualNeural --rate="+20%')
        }
    }
